...
describe("Primeiro Container de Teste", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      `⏱️  beforeAll completo em: ${(performance.now() - startTime).toFixed(
        2
      )}ms\n`
    );
  });
  ...
  afterEach((context) => {
    ...
    console.log(
      `⏱️  "${context.task.name}" executado em: ${duration.toFixed(2)}ms`
    );
  });
  ...
  afterAll(async () => {
    ...
    console.log(
      `\n⏱️  Teardown completo em: ${(performance.now() - startTime).toFixed(
        2
      )}ms`
    );
  });
