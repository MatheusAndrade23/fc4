...
// ========== FUNÇÕES AUXILIARES ==========
async function criarPostgresContainer(): Promise<{
  ...
  console.log(
    `PostgreSQL container iniciado em: ${container.getHost()}:${container.getMappedPort(
      5432
    )}`
  );
...
async function criarRedisContainer(): Promise<{
  container: StartedTestContainer;
  ...
  console.log(
    `Redis container iniciado em: ${container.getHost()}:${container.getMappedPort(
      6379
    )}`
  );
...
describe("PostgreSQL e Redis", () => {
  ...
  test.concurrent(
    "deve conseguir criar uma tabela e inserir dados no PostgreSQL",
    async () => {
      const { container: postgresContainer, pool: pgPool } =
        await criarPostgresContainer();
  ...
  test.concurrent(
    "deve conseguir armazenar e recuperar dados no PostgreSQL",
    async () => {
      const { container: postgresContainer, pool: pgPool } =
        await criarPostgresContainer();
  ...
  test.concurrent(
    "deve conseguir armazenar e recuperar valores no Redis",
    async () => {
      const { container: redisContainer, client: redisClient } =
        await criarRedisContainer();
